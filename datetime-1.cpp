...
unsigned int LunarCalendarTable[199] =  {  
    ...
    int Spring_NY,Sun_NY,StaticDayCount;  //Spring_NY 记录春节离当年元旦的天数,Sun_NY 记录阳历日离当年元旦的天数。
    int index,flag,mark=0;    //StaticDayCount记录大小月的天数 29或30; index 记录从哪个月开始来计算;flag 是用来对闰月的特殊处理。
	...
	const char *Day[] = {"*","初一","初二","初三","初四","初五","初六","初七","初八","初九","初十",  
                           "十一","十二","十三","十四","十五","十六","十七","十八","十九","二十",  
                           "廿一","廿二","廿三","廿四","廿五","廿六","廿七","廿八","廿九","三十"};  
    const char *Month[] = {"*","正","二","三","四","五","六","七","八","九","十","冬","腊"};
    printf("当前农历时间："); 
     if (mark){  
        printf("闰");  
        printf("%s月",Month[(LunarCalendarDay & 0x3C0) >> 6]);  
    }  
    else  
        printf("%s月",Month[(LunarCalendarDay & 0x3C0) >> 6]);    
